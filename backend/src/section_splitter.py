...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Extract outlines from PDFs in a directory.")
    parser.add_argument("--input-dir", type=Path, required=True, help="Directory containing the PDF files to process.")
    parser.add_argument("--output-dir", type=Path, required=True, help="Directory where the output JSON files will be saved.")
    ...
    files_to_process = sorted(list(args.input_dir.glob("*.pdf")))
    if not files_to_process:
        print(f"[WARN] No PDF files found in {args.input_dir}")
        sys.exit(0)

    print(f"[INFO] Found {len(files_to_process)} PDF(s) to process for outlines.")
    for pdf_path in files_to_process:
        try:
            process_pdf(pdf_path, args.output_dir, debug_print=debug_mode)
        except Exception as e:
            print(f"[CRITICAL ERROR] Failed to process {pdf_path.name} for outline: {e}")
            import traceback
            traceback.print_exc()